...
//                product.setBookNoticeList(oldProduct.getBookNoticeList());
        ...
        if(detailBaseResult != null && detailBaseResult.getData() != null){
            ...
            if(existScenic != null){
                ...
                if(StringUtils.isBlank(existScenic.getDetailDesc()) && StringUtils.isNotBlank(scenicDetail.getScenicDescription())){
                    existScenic.setDetailDesc(StringUtil.replaceImgSrc(StringUtil.delHTMLTag(scenicDetail.getScenicDescription())));
                ...
                if(ListUtils.isEmpty(existScenic.getImages()) && StringUtils.isNotBlank(scenicDetail.getDefaultPic())){
                    existScenic.setImages(Lists.newArrayList(UploadUtil.getNetUrlAndUpload(scenicDetail.getDefaultPic())));
    ...
    private ScenicSpotRuleMPO saveRule(ScenicSpotProductMPO scenicSpotProductMPO, DfyTicketDetail dfyTicketDetail, boolean fresh){
        ...
        if(scenicSpotProductBackupMPO != null){
            ...
            if(!StringUtils.equals(backup.getMpLossInfo(), dfyTicketDetail.getMpLossInfo())){
                ...
                ruleMPO.setRefundRuleDesc(checkRefundDesc(StringUtil.delHTMLTag(dfyTicketDetail.getMpLossInfo())));
            }
    ...
    private String checkRefundDesc(String desc){
...
//        }
        // 产品要求添加笛风云默认退改说明
        if(StringUtils.isBlank(groupTourProductMPO.getRefundDesc())){
            groupTourProductMPO.setRefundDesc("(1)出发前7 日及以上的，支付旅游费用总额5%的违约金;<br>(2)出发前6日至4\n" +
                    "日的，支付旅游费用总额10%的违约金;<br>(3)出发前3日至1日的，支付旅游费用总额15%的违约金;<br>(4)出发当日的，支付旅游费用总额20%的违约金。\n" +
                    "上述款项供应商有权选择在双方合作的后续款项中予以直接扣除，且开具的发票金额按照扣除后金额开具。<br>遇到国家法定节假日出游的客人，因个人原因退团以实际损失为准(损失较大甚至全损)。");
        }
                ...
                setMealMPO.setGroupTourTripInfos(journeyInfo.getJourneyDescJson().getData().getData().stream().map(j -> {
                    ...
                    if(j.getTraffic() != null){
                        ...
                        if(ListUtils.isNotEmpty(j.getTraffic().getToList())){
                            j.getTraffic().getToList().stream().distinct().forEach(t -> sb.append("-").append(t.getTo()));
                        }
                        ...
                        for (DfyJourneyDetail.JourneyModule journeyModule : j.getModuleList()) {
                            ...
                            if (type == DfyConstants.MODULE_TYPE_SCENIC &&
                                ...
                                for (DfyJourneyDetail.ModuleScenic scenic : journeyModule.getScenicList()) {
                                    ...
                                    // 计算一下转换成hh:mm
                                    item.setPlayTime(convertTime(scenic.getTimes()));
                                    item.setType("1");
                                    item.setPoiName(scenic.getTitle());
                                    item.setPoiDesc(StringUtil.delHTMLTag(scenic.getContent()));
                            ...
                            if (type == DfyConstants.MODULE_TYPE_HOTEL
                                ...
                                List<GroupTourHotel> groupTourHotels = journeyModule.getHotelList().stream().map(h -> {
                                    ...
                                    groupTourHotel.setDesc(StringUtil.delHTMLTag(h.getDescription()));
                                    ...
                                    if(ListUtils.isNotEmpty(h.getRoom())){
                                        ...
                                        groupTourHotel.setDesc(String.format("%s<br>%s", StringUtils.isBlank(groupTourHotel.getDesc()) ? "" : StringUtil.delHTMLTag(groupTourHotel.getDesc()),
                                                h.getRoom().stream().filter(r ->
                                                        StringUtils.isNotBlank(r.getDescription())).map(r ->
                                                        String.format("%s<br>%s<br>", r.getTitle(), StringUtil.delHTMLTag(r.getDescription()))).collect(Collectors.joining())));
                                    }
                                ...
                                if(ListUtils.isNotEmpty(groupTourHotels)){
                                    if(groupTourHotels.stream().allMatch(h -> StringUtils.isBlank(h.getDesc()))){
                                        groupTourHotels.get(0).setDesc(StringUtil.delHTMLTag(journeyModule.getDescription()));
                                    }
                            ...
                            if (type == DfyConstants.MODULE_TYPE_TRAFFIC && journeyModule.getTraffic() != null) {
                                GroupTourProductTripItem item = new GroupTourProductTripItem();
                                item.setPlayTime(convertTime(journeyModule.getTraffic().getTimes()));
                                String means = "";
...
//                                    item.setPoiName(String.format("从%s到%s", journeyModule.getTraffic().getFrom(), journeyModule.getTraffic().getTo()));
                                ...
                                item.setPoiDesc(StringUtil.delHTMLTag(journeyModule.getDescription()));
                            ...
                            if (type == DfyConstants.MODULE_TYPE_FOOD && journeyModule.getFood() != null) {
                                ...
                                if(ListUtils.isNotEmpty(journeyModule.getFood().getHasList())
                                    ...
                                    item.setSubType("20");
                                    item.setTime(journeyModule.getMoment());
                                    item.setPlayTime(convertTime(journeyModule.getFood().getTimes()));
                                    item.setType("3");
                            ...
                            if (type == DfyConstants.MODULE_TYPE_SHOPPING && ListUtils.isNotEmpty(journeyModule.getShopList())) {
                                for (DfyJourneyDetail.ModuleShop moduleShop : journeyModule.getShopList()) {
                                    ...
                                    item.setPoiDesc(StringUtil.delHTMLTag(moduleShop.getInstruction()));
                                    if(StringUtils.isNotBlank(moduleShop.getProduct())){
                                        item.setPoiDesc(String.format("%s<br>%s", item.getPoiDesc() == null ? "" : item.getPoiDesc(), moduleShop.getProduct()));
                                    }
                            ...
                            if (type == DfyConstants.MODULE_TYPE_ACTIVITY && journeyModule.getActivity() != null) {
                                ...
                                item.setPlayTime(convertTime(journeyModule.getActivity().getTimes()));
                                item.setPoiDesc(StringUtil.delHTMLTag(journeyModule.getDescription()));
                                items.add(item);
                            }
                            if (type == DfyConstants.MODULE_TYPE_REMINDER && journeyModule.getRemind() != null) {
                                ...
                                item.setPoiDesc(StringUtil.delHTMLTag(journeyModule.getRemind().getContent()));
                                items.add(item);
                            }
                        }
                    }
    ...
    private String removePre(String str){
    ...
    private String convertTime(Integer time){
        if(time != null){
            int hh = time / 60;
            int mm = time % 60;
            return String.format("%s:%s", hh >= 10 ? hh : String.format("0%s", hh), mm >= 10 ? mm : String.format("0%s", mm));
        }
        return null;
    }
    ...
    private void setCity(GroupTourProductMPO groupTourProductMPO, DfyToursDetailResponse dfyToursDetail){
        if(ListUtils.isNotEmpty(dfyToursDetail.getDepartCitys())){
            groupTourProductMPO.setDepInfos(dfyToursDetail.getDepartCitys().stream().map(d ->
                    commonService.setCity(null, d.getName(), null)).filter(a -> a != null).distinct().collect(Collectors.toList()));
        }
        if(ListUtils.isNotEmpty(dfyToursDetail.getDesPoiNameList())){
            groupTourProductMPO.setArrInfos(dfyToursDetail.getDesPoiNameList().stream().map(d ->
                    commonService.setCity(d.getDesProvinceName(), d.getDesCityName(), d.getDesCountyName())).distinct().collect(Collectors.toList()));
        }
    }
